corPercent = [0.001, 0.1, 1 ,3, 6, 10].*10;%10mV sigma_vtf so muitplied by 10

x2 = [corPercent, fliplr(corPercent)];%can be reused for all other plotting!

%reading the symmetric circuit data
for i = 1:5
    corsymmetricData(:,:, i) = csvread("D:\perturbation_on_chip_learning\Perturbation-techniques-in-CNNs\dataforpaper\testData\hybridRev\Sym\"+string(i-1)+".csv");
end
...
for i = 1:5
    corasymmetricData(:,:, i) = csvread("D:\perturbation_on_chip_learning\Perturbation-techniques-in-CNNs\dataforpaper\testData\hybridRev\Asym\"+string(i-1)+".csv");
end
